...
public class Day04_6pr {
	...
	public static void main(String[] args) {
		...
		// 게임실행
		while(true) { // 무한루프 종료조건 : 1. 9칸 모두 알이 있을경우 2. 승리자가 나왔을경우 3. 무승부
			...
			while(true) {
				...
				if( 승리알.equals("[O]") ) {
					break; // 승리알 조건이 나왔다면 게임종료
				}
				else if( 게임판[위치].equals("[ ]") ) { // 3. 해당 난수위치가 공백이면
					// 난수 위치에 공백이면 X 알 두기
			...
			//////////////// 5. 무승부 //////////////////////
			//조건 알이 꽉차있는데 승리알 조건이 안나왔을때
			int 위치 = random.nextInt(9);
			if( 게임판[위치].equals("[O]") && 승리알.equals("[X]") )
			if( !게임판.equals("[ ]") ) {
			}
			if( !승리알.equals("[O]") && !승리알.equals("[X]") ) {
			}
